...
class PollerKqueue : public PollerImpl {
  public:
    PollerKqueue(EventLoop* loop);
  public:
  ...
  private:
    const static size_t MAX_KQUEUE_EVENTS = 1024;
  private:
    std::array<struct kevent, MAX_KQUEUE_EVENTS> events_;
    int kfd_;
};
}
}
